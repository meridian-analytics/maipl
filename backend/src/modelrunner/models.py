...
class ModelRunnerTask(models.Model):
    ...
    threshold = models.FloatField(null=True, default=0.5, help_text='The threshold value used to determine the cut-off point for detections')
    step_size = models.FloatField(null=True, default=None, help_text='Step size in seconds')
    batch_size = models.IntegerField(null=True, default=32, help_text='How many samples will be loaded into memory')
    merge_detections = models.BooleanField(null=True, default=False, help_text='A flag indicating whether to merge overlapping detections into a single detection.')
    overwrite = models.BooleanField(null=True, default=True, help_text='Overwrites the detections, otherwise appends to it')
    buffer = models.FloatField(null=True, default=0.0, help_text='The buffer duration to be added to each detection in seconds')
