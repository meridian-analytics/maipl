#!/usr/bin/env python3
"""
Dynamic inventory script for Database PROD environment.
"""
import json
import subprocess
import os
...
def build_inventory(tf_output):
    group_name = f'database_{ENVIRONMENT}'
    inventory = {
        '_meta': {'hostvars': {}},
        group_name: {
            'hosts': [],
    ...
    # Get instance IPs and names from Terraform output
    instance_ips = tf_output.get('database_instance_ips', {}).get('value', {})
    instance_names = tf_output.get('database_instance_names', {}).get('value', {})
    
    # Add each instance to the inventory
    for idx in instance_ips:
